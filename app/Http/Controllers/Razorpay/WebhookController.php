...
class WebhookController extends CommissionController
{
    public function confirmPayout(Request $request)
    {
        ...
        DB::transaction(function () use ($request){
            ...
            $payout = $payout->get();
            if ($payout[0]->status == 'processed' || $payout[0]->status == 'reveresed' || $payout[0]->status == 'cancelled' || $payout[0]->status == 'failed' || $payout[0]->status == 'processing') {
                $array = [
                    'status' => true,
            ...
            if ($request['payload.payout.entity.status'] == 'cancelled' || $request['payload.payout.entity.status'] == 'processing' || $request['payload.payout.entity.status'] == 'rejected') {
                $result = $data->get();
