...
class WebhookController extends CommissionController
{
    public function confirmPayout(Request $request)
    {
        ...
        if ($request['payload.payout.entity.status'] == 'processed') {
            $result = $data->get();
            $commission = $this->baseCommission($result[0]->amount / 100, $result[0]->user_id);
        }
        if ($request['payload.payout.entity.status'] == 'reversed') {
            ...
            $this->transaction(0, "Payout Reversal", 'dmt', $result[0]->user_id, $opening_balance, $transaction_id, $closing_balance, $request['payload.payout.entity.amount'] / 100);
