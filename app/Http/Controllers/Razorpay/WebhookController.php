    ...
    public function confirmPayout(Request $request)
    {
        ...
        DB::transaction(function () use ($request) {
            ...
            if ($request['payload.payout.entity.status'] == 'cancelled' || $request['payload.payout.entity.status'] == 'failed' || $request['payload.payout.entity.status'] == 'rejected') {
                ...
                $this->generalCallbackTransaction(0, "Payout Reversal for account $account_number", 'payout', $result[0]->user_id, $user->wallet, $request['payload.payout.entity.reference_id'], $user->wallet + $request['payload.payout.entity.amount'] / 100, json_encode($metadata), $request['payload.payout.entity.amount'] / 100);
