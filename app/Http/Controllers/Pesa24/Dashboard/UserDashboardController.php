...
class UserDashboardController extends Controller
{
    ...
    public function transactionLedger(Request $request, $name = null)
    {
        ...
        if (!empty($search) || !is_null($search)) {
            $data = DB::table('transactions')->where('trigered_by', $id)->where('transaction_id', 'like', '%' . $search . '%')->orWhere('transaction_for', 'like', '%' . $search . '%')->orWhereJsonContains('metadata->status', 'like', '%' . $search . '%')->latest()->orderByDesc('transactions.id')->get();
            return $data;
        }
