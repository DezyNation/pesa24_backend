...
class UserDashboardController extends Controller
{
    ...
    public function transactionLedger(Request $request, $name = null)
    {
        ...
        if (!empty($search) || !is_null($search)) {
            $data = DB::table('transactions')->where('trigered_by', auth()->user()->id)->orWhere('user_id', auth()->user()->id)->where('transaction_for', 'like', '%' . $search . '%')->orWhere('transaction_id', 'like', '%' . $search . '%')->latest()->paginate(200)->appends(['from' => $request['from'], 'to' => $request['to'], 'status' => $request['status']]);;
            return $data;
        }
        ...
        $data = DB::table('transactions')->whereBetween('created_at', [$request['from'] ?? Carbon::now()->startOfDecade(), $request['to'] ?? Carbon::now()->endOfDecade()])->where('trigered_by', auth()->user()->id)->orWhere('user_id', auth()->user()->id)->latest()->paginate(200)->appends(['from' => $request['from'], 'to' => $request['to'], 'status' => $request['status']]);
        return $data;
    }
