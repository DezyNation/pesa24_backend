...
class FundController extends Controller
{
    ...
    public function pendingfetchFund(Request $request, $type)
    {
        if ($type == 'pending') {
            $data = DB::table('funds')->join('users', 'users.id', '=', 'funds.user_id')
                ->join('users as admin', 'admin.id', '=', 'funds.parent_id')
                ->whereBetween('funds.created_at', [$request['from'] ?? Carbon::today(), $request['to'] ?? Carbon::tomorrow()])
                ->where(['users.organization_id' => auth()->user()->organization_id, 'funds.status' => 'pending'])->select('funds.*', 'funds.id as fund_id', 'users.name', 'users.phone_number', 'admin.name as admin_name', 'admin.id as admin_id')->latest('funds.updated_at')->paginate(100);
            return $data;
        }
        $data = DB::table('funds')->join('users', 'users.id', '=', 'funds.user_id')
            ->join('users as admin', 'admin.id', '=', 'funds.parent_id')
            ->whereBetween('funds.created_at', [$request['from'] ?? Carbon::today(), $request['to'] ?? Carbon::tomorrow()])
            ->where(['users.organization_id' => auth()->user()->organization_id])->where('funds.status', '!=', 'pending')->select('funds.*', 'funds.id as fund_id', 'users.name', 'users.phone_number', 'admin.name as admin_name', 'admin.id as admin_id')->latest('funds.updated_at')->paginate(100);
        return $data;
    }
    ...
    public function reversalAndTransferFunds(Request $request)
    {
        $data = DB::table('funds')->join('users', 'users.id', '=', 'funds.user_id')
            ...
            ->whereBetween('funds.created_at', [$request['from'] ?? Carbon::today(), $request['to'] ?? Carbon::tomorrow()])
            ->select('users.name', 'users.phone_number', 'funds.transaction_id', 'funds.user_id', 'funds.amount', 'funds.remarks', 'funds.transaction_type', 'funds.created_at', 'admin.name as admin_name', 'admin.id as admin_id', 'funds.id', 'admin.phone_number as admin_phone')
